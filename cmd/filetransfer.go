...
var fileTransferCmd = &cobra.Command{
	...
	Short: "Transfer files from/to device(s)",
...
func (c *NewFileTransferCmd) Run() error {
	...
	m := &ws.ProtoMsg{
		Header: ws.ProtoHdr{
			...
			MsgType:    filetransfer.MessageTypeStat,
	...
	m = &ws.ProtoMsg{
		Header: ws.ProtoHdr{
			Proto:      ws.ProtoTypeFileTransfer,
			MsgType:    filetransfer.MessageTypeGet,
			Properties: map[string]interface{}{},
		},
		Body: []byte(filePathSource),
	}
...
func getWebSocketScheme(schema string) string {
	switch schema {
	case "http":
		return "ws"
	case "https":
		return "wss"
	default:
		// Do wss on unrecognized schemas (?)
		return "wss"
	}
}
